...
interface CardCreatorDependencies extends
    Dependency<'HTMLBodyElement', typeof HTMLBodyElement>,
    Dependency<'crypto', Pick<typeof crypto, 'randomUUID'>>
{}

class CardCreator {
    static definition({HTMLBodyElement, crypto}: CardCreatorDependencies) {
        return new CustomElementDefinition('card-creator', class extends HTMLBodyElement {
            constructor() {
                super();
...
(async () => {
    const app = LazyMap.create()
        ...
        .set('HTMLBodyElement', instance(HTMLBodyElement))
