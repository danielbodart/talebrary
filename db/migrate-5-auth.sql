-- Step 5: Better Auth tables (user/session/account/verification).
-- Generated by `@better-auth/cli generate` (better-auth 1.6.x, sqlite dialect).
-- The dash() dashboard plugin is hosted and adds no local tables.
-- Apply to local db/talebrary.sqlite AND remote D1 (wrangler d1 execute --remote).
